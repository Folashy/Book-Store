...
export async function RegisterAuthor(req:Request, res:Response, next:NextFunction) {
    const id = uuidv4()
    try{ 
        ...
        const duplicatEmail = await AuthorInstance.findOne({where:{email:req.body.email}})
        if(duplicatEmail){
 ...
 export async function LoginAuthor(req:Request, res:Response, next:NextFunction) {
   ...
   try{ 
       ...
       const User = await AuthorInstance.findOne({where:{email:req.body.email}}) as unknown as {[key:string]:string}
...
export async function getAuthors(
   ...
   try {
     ...
     const record = await BookInstance.findAndCountAll({ limit, offset,
     include:[{
      model:BookInstance,
